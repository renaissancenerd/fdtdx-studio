...
class Pop_up_constraints():
  ...
  def open_pop_up(self, typ, other_object_list, Con={},):
  ...
  def remove_tuple(self, Con, typ):
    ...
    for key, value in Con.items():
      if key == 'axes':
        for i in range(len(coor)): self.con_value[(key + '_' + coor[i])] = True if i in value else False
      elif isinstance(value, (list, tuple)) and key != 'axes':
        for i in range(len(coor)): self.con_value[(key + '_' + coor[i])] = value[next(a, 0)] if i in Con['axes'] else None
  ...
  def build_pos_con(self):
    ui.select(options=self.other_object_list,
              with_input=True,
  ...
  def build_grid_con(self):
    ...
    with ui.row():
      ...
      with ui.column().classes('flex-1').props('dense').bind_visibility_from(show_x, 'value'):
        ui.toggle({0: '-', 1: '+'}).bind_value(self.con_value, 'sides_x',forward=lambda v: None if v is None else ('-' if v == 0 else '+')
      ...
      with ui.column().classes('flex-1').props('dense').bind_visibility_from(show_y, 'value'):
        ui.toggle({0: '-', 1: '+'}).bind_value(self.con_value, 'sides_y',forward=lambda v: None if v is None else ('-' if v == 0 else '+')
                                                ,backward=lambda v: None if v is None else (0 if v == '-' else 1))
      ...
      with ui.column().classes('flex-1').props('dense').bind_visibility_from(show_z, 'value'):
        ui.toggle({0: '-', 1: '+'}).bind_value(self.con_value, 'sides_z',forward=lambda v: None if v is None else ('-' if v == 0 else '+')
                                                ,backward=lambda v: None if v is None else (0 if v == '-' else 1))
  ...
  def build_size_con(self):
    ...
    with ui.row().classes('w-full flex-nowrap gap-1'):
        ...
        with ui.column().classes('pt-3'):
          ...
          #Column for X-con_value
        with ui.column().bind_visibility_from(show_x, 'value').classes('flex-1').props('dense'):
          ui.number().bind_value(self.con_value, 'other_axes_x', forward=lambda v: int(v) if v is not None else None)
          ...
          #Column for Y-con_value
        with ui.column().bind_visibility_from(show_y, 'value').classes('w-1/4').classes('flex-1').props('dense'):
          ui.number().bind_value(self.con_value, 'other_axes_y', forward=lambda v: int(v) if v is not None else None)
          ...
          #Column for Z-con_value
        with ui.column().bind_visibility_from(show_z, 'value').classes('w-1/4').classes('flex-1').props('dense'):
          ui.number().bind_value(self.con_value, 'other_axes_z', forward=lambda v: int(v) if v is not None else None)
  ...
  def build_size_ex_con(self):
    ...
    with ui.row():
      ...
      with ui.column():
        ...
        ui.toggle({0: '-', 1: '+'}).props('inline').bind_value(self.con_value, 'direction', forward=lambda v: None if v is None else ('-' if v == 0 else '+'),backward=lambda v: None if v is None else (0 if v == '-' else 1))
  ...
  def build_make_new_con(self):
    ...
    with ui.stepper().props('vertical').classes('w-full') as self.stepper:
      with ui.step('choose Constrains Typ'):
        ...
        ui.button('Next', on_click=lambda: self.create_in_step(step2) if self.con_value['type'] in the_list else ui.notification('you need to pick a ConstraintTyp'))
