...
class pop_up_new_material(NewPopUp):
  ...
  def checkButton(self,permittivity, permeability,e_conductivity, m_conductivity, name, label):
    obj= self.controller.model.material.material_exists_settings(permeability= permeability,permittivity= permittivity, e_conductivity= e_conductivity, m_conductivity= m_conductivity)
    if obj == -1:
      self.add_material(Permittivity=permittivity,Permeability= permeability, Electrical_conductivity=e_conductivity, Magnetic_conductivity= m_conductivity,name=name)
    else:
  ...
  def make_source_mode_options(self):
    ...
    if self.input_kind == 'Scalar':
      ...
      e_cond= ui.number(value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/2')
      ui.label('Magnetic_conductivity').style('font-size: 13px; font-weight: bold;')
      m_cond= ui.number(value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/2')
      Error =ui.label('Material with the same Settings already exists').style('color: red; font-size: 13px;')
      Error.set_visibility(False)
      self.save = ui.button('Save', on_click= lambda: self.checkButton(permittivity=pitty.value, permeability= perm.value, e_conductivity=e_cond.value, m_conductivity=m_cond.value,name= name,label= Error))
    else:
      ui.label('Name').style('font-size: 13px; font-weight: bold;')
      ui.input(value= 'Unknown Material', on_change= lambda e: setName(e.value))
      ui.label('Permittivity').style('font-size: 13px; font-weight: bold;')
      ...
      with ui.row().classes('w-full flex-nowrap gap-1') as electric_conductivity:
        e_condX= ui.number('x',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
        e_condY= ui.number('y',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
        e_condZ= ui.number('z',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
      ui.label('Magnetic_conductivity').style('font-size: 13px; font-weight: bold;')
      with ui.row().classes('w-full flex-nowrap gap-1') as magnetic_conductivity:
        m_condX= ui.number('x',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
        m_condY= ui.number('y',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
        m_condZ= ui.number('z',value= 0.0, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('w-1/6')
      self.save = ui.button('Save', on_click= lambda: self.add_material([pittyX.value,pittyY.value,pittyZ.value],[permX.value,permY.value,permZ.value],[e_condX.value,e_condY.value,e_condZ.value],[m_condX.value,m_condY.value,m_condZ.value],name))
