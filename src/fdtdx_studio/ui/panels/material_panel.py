...
class Material_panel(ObjectConfigPanel):
  ...
  def checkButton(self,permittivity, permeability,e_conductivity, m_conductivity, name, label):
    ...
    if obj == -1:
      ...
      self.update_material(permittivity=permittivity,permeability= permeability, electric_conductivity=e_conductivity, magnetic_conductivity= m_conductivity,name=name)
    ...
    with panel:
      with ui.column().classes('w-full gap-2 p-2'):
        ...
        with ui.row().classes('w-full flex-nowrap gap-1'):
          permittivityX = ui.number('x', value= self.permittivityX, min= 0, step= 0.1, validation= self._validate).classes('flex-1').props('dense').bind_enabled_from(self, 'is_readonly', backward=lambda x: not x)
        ...
        with ui.row().classes('w-full flex-nowrap gap-1'):
          permeabilityX = ui.number('x', value= self.permeabilityX, min= 0, step= 0.1, validation= self._validate).classes('flex-1').props('dense').bind_enabled_from(self, 'is_readonly', backward=lambda x: not x)
        ...
        with ui.row().classes('w-full flex-nowrap gap-1'):
          electric_conductivityX = ui.number('x', value= self.electric_conductivityX, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('flex-1').props('dense').bind_enabled_from(self, 'is_readonly', backward=lambda x: not x)
        ...
        with ui.row().classes('w-full flex-nowrap gap-1'):
          magnetic_conductivityX = ui.number('x', value= self.magnetic_conductivityX, min= 0, max= 1.0, step= 0.1, validation= self._validate_conductivity).classes('flex-1').props('dense').bind_enabled_from(self, 'is_readonly', backward=lambda x: not x)
        ...
        if self.is_readonly == False:
          ...
          self.apply = ui.button('Apply', on_click=lambda: self.checkButton(name=name.value,permittivity= permittivityX.value,permeability= permeabilityX.value, e_conductivity=electric_conductivityX.value, m_conductivity=magnetic_conductivityX.value, label= Error))
  ...
  def update_material(self, name, permittivity, permeability, electric_conductivity, magnetic_conductivity):
    ...
    material = fdtdx.Material(permittivity= permittivity, permeability= permeability, electric_conductivity=electric_conductivity,magnetic_conductivity=magnetic_conductivity)
