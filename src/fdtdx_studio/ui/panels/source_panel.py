...
class SourcePanel(ObjectConfigPanel):
  ...
  def render_into(self, panel):
        ...
        with panel:
          ...
          ui.label('Direction & Angles').style('font-size: 14px; padding-bottom: 0px; font-weight: bold; margin-top: 12px;').tooltip("Sets the direction and angles of the source.")
          with ui.column().style('padding-left: 4px').classes('w-full'):
            ...
            self.azimuth_angle = ui.number('Azimuth Angle', validation=self._validate_float).classes('flex-1').props('dense')
            self.elevation_angle = ui.number('Elevation Angle', validation=self._validate_float).classes('flex-1').props('dense')
            self.max_angle_random_offset = ui.number('Max Angel Random Offset', validation=self._validate_float).classes('flex-1').props('dense')

          # Horizontal Offset
          ui.label('Horizontal Offset').style('font-size: 14px; padding-bottom: 0px; font-weight: bold; margin-top: 12px;').tooltip("Maximum horizontal offset of the source.")
          with ui.column().classes('w-full'):
            self.max_horizontal_offset = ui.number('Max Offset', validation=self._validate_float).classes('flex-1').props('dense')

          # Amplitude
          ui.label('Static Amplitude Factor').style('font-size: 14px; padding-bottom: 0px; font-weight: bold; margin-top: 12px;').tooltip("Static amplitude factor for the source.")
          with ui.column().style('padding-left: 8px').classes('w-full'):
            self.static_amplitude_factor = ui.number('Factor', validation=self._validate_float).classes('flex-1').props('dense')

          # Temporal Profile
          ui.label('Temporal Profile').style('font-size: 14px; padding-bottom: 0px; font-weight: bold; margin-top: 12px;').tooltip("Temporal profile settings (phase shift and startup periods).")
          self.temporal_profile_button = ui.select(['SingleFrequencyProfile', 'GaussianPulseProfile'], on_change=lambda: self.set_temp_profile_type(), label='Profile Type', value='SingleFrequencyProfile').classes('w-3/4')  
          with ui.column().style('padding-left: 6px').classes('w-full') as self.temp_profile_display:
            self.temp_profile_value1 = ui.number('Phase Shift', validation=self._validate_float).classes('flex-1').props('dense')
            self.temp_profile_value2 = ui.number('Num of Startup Periods', validation=self._validate_float).classes('flex-1').props('dense')
  ...
  def get_parameters(self): 
        ...
        parameters['static_amplitude_factor'] = self.static_amplitude_factor.value if self.static_amplitude_factor else None
