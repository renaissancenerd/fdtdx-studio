...
class SwitchElement:
  ...
  def build_on_off_switch_panel(self):
        ...
        ui.label('OnOffSwitch Configuration') \
        ...
        self.start_after_periods = ui.number(
            label='Start After Periods',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
          .tooltip('Turn on after this many source periods have elapsed')

        self.end_time = ui.number(
            label='End Time',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
        ...
        self.end_after_periods = ui.number(
            label='End After Periods',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
          .tooltip('Turn off after this many source periods from start')

        self.on_for_time = ui.number(
            label='On For Time',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
        ...
        self.on_for_periods = ui.number(
            label='On For Periods',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
          .tooltip('Duration (in source periods) the detector stays on')

        self.period = ui.number(
            label='Period',
            format='%.6f',
            value=None
        ).props('clearable').classes('w-full') \
        ...
        self.fixed_on_time_steps = ui.input(
            label='Fixed On Time Steps',
            placeholder='e.g., 100, 250, 500'
        ).classes('w-full') \
          .tooltip('Force detector ON at these exact time steps (comma-separated)')

        self.is_always_off = ui.checkbox('Is Always Off') \
            .tooltip('Completely disable the detector regardless of other settings')
