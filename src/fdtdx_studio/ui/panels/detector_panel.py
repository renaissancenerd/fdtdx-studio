...
from fdtdx.core.jax.pytrees import frozen_field
...
class DetectorConfigurationPanel(ObjectConfigPanel):
    ...
    def render_specific_parameters(self, panel):
        ...
        with panel:
            ...
            # === Data Type ===
            ui.label('Data Type') \
                .style('font-size: 14px; font-weight: bold; margin-top: 16px; margin-bottom: 4px') \
            ...
            # === Boolean Flags ===
            ui.label('Flags') \
                .style('font-size: 14px; font-weight: bold; margin-top: 20px; margin-bottom: 8px') \
                .tooltip('Various boolean options affecting detector behavior')

            self.exact_interpolation = ui.checkbox('Exact Interpolation') \
                .tooltip('Use exact (slower) interpolation instead of fast approximation')
            self.inverse = ui.checkbox('Inverse') \
                .tooltip('Record the inverse field (e.g., 1/E instead of E)')
            self.if_inverse_plot_backwards = ui.checkbox('If Inverse Plot Backwards') \
                .tooltip('When in inverse mode, reverse the time axis in plots')

            # Video & Plotting
            ui.label('Video & Plotting') \
                .style('font-size: 14px; font-weight: bold; margin-top: 24px; margin-bottom: 8px')

            self.num_video_workers = ui.number(
                label='Number of Video Workers',
                value=0,
                min=0
            ).classes('w-full') \
             .tooltip('Number of parallel workers for video generation (0 = auto)')

            self.plot_interpolation = ui.input(
                label='Plot Interpolation Method',
                value='gaussian'
            ).classes('w-full') \
    ...
    def get_parameters(self):
        ...
        parameters["dtype"] = self.dtype.value if getattr(self, 'dtype', None) is not None and hasattr(self.dtype, 'value') else (self.dtype if self.dtype is not None else None)
        parameters["exact_interpolation"] = self.exact_interpolation.value if getattr(self, 'exact_interpolation', None) is not None and hasattr(self.exact_interpolation, 'value') else (self.exact_interpolation if self.exact_interpolation is not None else None)
        parameters["inverse"] = self.inverse.value if getattr(self, 'inverse', None) is not None and hasattr(self.inverse, 'value') else (self.inverse if self.inverse is not None else None)
        parameters["if_inverse_plot_backwards"] = self.if_inverse_plot_backwards.value if getattr(self, 'if_inverse_plot_backwards', None) is not None and hasattr(self.if_inverse_plot_backwards, 'value') else (self.if_inverse_plot_backwards if self.if_inverse_plot_backwards is not None else None)
        parameters["num_video_workers"] = self.num_video_workers.value if getattr(self, 'num_video_workers', None) is not None and hasattr(self.num_video_workers, 'value') else (self.num_video_workers if self.num_video_workers is not None else None)
        parameters["plot_interpolation"] = self.plot_interpolation.value if getattr(self, 'plot_interpolation', None) is not None and hasattr(self.plot_interpolation, 'value') else (self.plot_interpolation if self.plot_interpolation is not None else None)
        if self.switch:
    ...
    def update_values(self, parameters):
        ...
        if getattr(self, 'if_inverse_plot_backwards', None) is not None and hasattr(self.if_inverse_plot_backwards, 'value'):
