...
class Material():
  ...
  def get_material_from_settings(self, permeability, permittivity, e_conductivity, m_conductivity, name = 'Unknown Material'):
    '''returns the material from material list that matches the settings. Creates a new one if it does not exist'''
    obj = self.material_exists_settings(permeability=permeability,permittivity=permittivity,e_conductivity=e_conductivity,m_conductivity=m_conductivity)
    if obj != -1:
      return obj[1]
    self.create_new_material(permittivity=permittivity,permability=permeability,e_conductivity=e_conductivity,m_conductivity=m_conductivity, name=name)
  ...
  def material_exists_settings(self, permeability, permittivity, e_conductivity, m_conductivity):
    ...
    for obj in self.material_list:
      if (obj[1].permeability == permeability and
          obj[1].permittivity == permittivity and
          obj[1].electric_conductivity == e_conductivity and
          obj[1].magnetic_conductivity == m_conductivity):
        return obj 
    return -1
