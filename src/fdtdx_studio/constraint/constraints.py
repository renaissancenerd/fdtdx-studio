from fdtdx import PositionConstraint, SizeConstraint, SizeExtensionConstraint, GridCoordinateConstraint 
...
class Constraints:
  ...
  def get_obj_constraints(self, object_name:str):
    ...
    return [{**vars(v), "type":type(v).__name__, "key":k} for k,v in self.constraints.items() if object_name == v.object and "hidden" not in k]
